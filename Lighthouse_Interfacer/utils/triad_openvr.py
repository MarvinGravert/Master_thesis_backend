...
class pose_sample_buffer():
    ...
    def append(self, pose_mat, t):
        ...
        self.pitch.append(
            180 / math.pi * math.atan(-1 * pose_mat[2][0] / math.sqrt(pow(pose_mat[2][1], 2) + math.pow(pose_mat[2][2], 2))))
...
class vr_tracking_reference(vr_tracked_device):
    def get_mode(self):
        return self.vr.getStringTrackedDeviceProperty(
            self.index, openvr.Prop_ModeLabel_String).decode('utf-8').upper()
...
class triad_openvr():
    ...
    def remove_tracked_device(self, tracked_device_index):
        if tracked_device_index in self.device_index_map:
            ...
            raise Exception("Tracked device index {} not valid. Not removing.".format(
